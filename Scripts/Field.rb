...
def pbIsGrassTag?(tag)
  return tag==PBTerrain::Grass ||
         ...
         (tag==PBTerrain::Sand && !Rejuv) ||
         (tag==PBTerrain::PokeSand && Rejuv)
end
...
def pbGetEnvironment
  ...
  if $PokemonGlobal && $PokemonGlobal.diving
    ...
    case $game_player.terrain_tag
      when PBTerrain::Grass       then return :Grass
      when PBTerrain::TallGrass   then return :TallGrass
      when PBTerrain::DeepWater, PBTerrain::Water then return :MovingWater
      when PBTerrain::StillWater  then return :StillWater
      when PBTerrain::Rock        then return :Rock
      when PBTerrain::Sand, PBTerrain::PokeSand then return :Sand
    end
    return :None
  end
end
...
def pbGenerateWildPokemon(species,level,sos=false)
  ...
  chances=[60,20,5] if !$Trainer.party[0].isEgg? &&
     ($Trainer.party[0].ability == (:COMPOUNDEYES || :SUPERLUCK))
  itemrnd=rand(100)
...
def pbWildBattle(species,level,variable=Variables[:BattleResult],canescape=true,canlose=false)
  ...
  if $PokemonGlobal.partner && genwildpoke.isbossmon
...
def pbWildBattleObject(pokemon,variable=Variables[:BattleResult],canescape=true,canlose=false)
  $game_switches[:In_Battle] = true
  if (Input.press?(Input::CTRL) && $DEBUG) || $Trainer.pokemonCount==0
    if $Trainer.pokemonCount>0
      Kernel.pbMessage(_INTL("SKIPPING BATTLE..."))
  ...
  handled=[nil]
  Events.onWildBattleOverride.trigger(nil,pokemon.species,pokemon.level,handled)
  if handled[0]!=nil
    $game_switches[:In_Battle] = false
    return handled[0]
  end
  ...
  scene=pbNewBattleScene
  if $PokemonGlobal.partner && pokemon.isbossmon
    othertrainer=PokeBattle_Trainer.new(
       $PokemonGlobal.partner[1],$PokemonGlobal.partner[0])
    othertrainer.id=$PokemonGlobal.partner[2]
    othertrainer.party=$PokemonGlobal.partner[3]
    ...
    battle=PokeBattle_Battle.new(scene,combinedParty,[pokemon],
       [$Trainer,othertrainer],nil)
    battle.fullparty1=true
    battle.partneritems = $PokemonGlobal.partner[4]
    battle.doublebattle=true
  else
    battle=PokeBattle_Battle.new(scene,$Trainer.party,[pokemon],
       $Trainer,nil)
  end
  battle.internalbattle=true
  battle.cantescape=!canescape
...
def pbDoubleWildBattle(species1,level1,species2,level2,variable=Variables[:BattleResult],canescape=true,canlose=false)
  if (Input.press?(Input::CTRL) && $DEBUG) || $Trainer.pokemonCount==0
    if $Trainer.pokemonCount>0
      Kernel.pbMessage(_INTL("SKIPPING BATTLE..."))
    end
...
Events.onStartBattle+=proc {|sender,e|
  ...
  for i in 0...$Trainer.party.length
    check=checkEvolutionEx($Trainer.party[i]){|pokemon,method,condition,evo|
      next [evo,condition] if method == :LandCritical
      next nil
    }
    $PokemonTemp.evolutionLandCritical[i] = check
  end
}
  ...
  if decision!=2 && decision!=5 # not a loss or a draw
    if $PokemonTemp.evolutionLandCritical
      for i in 0...$PokemonTemp.evolutionLandCritical.length
        next if $PokemonTemp.evolutionLandCritical[i].nil?
        poke = $Trainer.party[i]
        next if poke.hp==0 || !poke.landCritical || poke.landCritical<$PokemonTemp.evolutionLandCritical[i][1]
        if poke && poke.landCritical>=$PokemonTemp.evolutionLandCritical[i][1]
          newspecies = $PokemonTemp.evolutionLandCritical[i][0]
          if !newspecies.nil?
...
Events.onStepTaken+=proc{
  ...
  if Reborn
    $game_switches[:Has_PokeSnax] = $PokemonBag.pbQuantity(:POKESNAX)>0 
  end
...
Events.onStepTakenFieldMovement+=proc{|sender,e|
  ...
  if sender.is_a?(Game_Character) && sender.respond_to?("id") && caller.any?{|c| c.to_s.include?("force_move_route")}
    if $PokemonMap && !sender.is_a?(Game_Player) && pbMapInterpreter.move_route_waiting == false
      $PokemonMap.addMovedEvent(sender.id)
    end
  end
}
...
def pbGetPlayerCharset(graphic,trainer=nil)
  player = trainer ? trainer.metaID : $PokemonGlobal.playerID
  trainer=$Trainer if !trainer
  outfit=trainer ? trainer.outfit : 0
  meta = $cache.metadata[:Players][player]
  ret=meta[graphic]
  ret=meta[:walk] if !ret || ret==""
...
def Kernel.pbUpdateVehicle
  if $PokemonGlobal.diving
    $game_player.character_name=pbGetPlayerCharset(:dive) # Diving graphic
  elsif $PokemonGlobal.surfing || $PokemonGlobal.lavasurfing
    $game_player.character_name=pbGetPlayerCharset(:surf) # Surfing graphic
  elsif $PokemonGlobal.bicycle
    $game_player.character_name=pbGetPlayerCharset(:bike) # Bicycle graphic
  else
    $game_player.character_name=pbGetPlayerCharset(:walk) # Regular graphic
  end
end
...
def pbCanUseBike?(mapid)
  val=$cache.mapdata[$game_map.map_id].Outdoor
  val=$cache.mapdata[$game_map.map_id].Bicycle if val==nil
...
def Kernel.pbMountBike
  ...
  Kernel.pbUpdateVehicle
  bikebgm=$cache.metadata[:Bicycle]
  pbSEPlay("Bike Bell")
  if bikebgm && $Settings.bike_and_surf_music == 1
...
Events.onMapChange+=proc {|sender,e|
  ...
  healing=$cache.mapdata[$game_map.map_id].HealingSpot
  ...
  if oldid!=0 && oldid!=$game_map.map_id
    weather = $cache.mapdata[$game_map.map_id].Weather
...
Events.onMapSceneChange+=proc{|sender,e|
  ...
  if $game_map
    lastmapdetails=$PokemonGlobal.mapTrail[0] ? $cache.mapdata[$PokemonGlobal.mapTrail[0]].MapPosition : [-1,0,0]
    lastmapdetails=[-1,0,0] if !lastmapdetails
    newmapdetails=$game_map.map_id ? $cache.mapdata[$game_map.map_id].MapPosition : [-1,0,0]
  ...
  darkmap=$cache.mapdata[$game_map.map_id].DarkMap
  if darkmap
    if $PokemonGlobal.flashUsed || (($game_switches[:EasyHMs_Password] || $PokemonBag.pbHasItem?(:GOLDENLANTERN)) && $PokemonBag.pbQuantity(:TM70)>0 && $Trainer.numbadges>=BADGEFORFLASH)
  ...
  if mapChanged
    if !$game_switches[:Disable_Signposts_Music] # for ending scenes
      if $cache.mapdata[$game_map.map_id].ShowArea
        nosignpost=false
        if $PokemonGlobal.mapTrail[1]
          ...
          oldmapnoshow=$cache.mapdata[$PokemonGlobal.mapTrail[1]].ShowArea
          oldmapnoshow=true if oldmapnoshow.nil?
...
def pbSetEscapePoint
  ...
  when 2; yco-=0; dir=8   # Down
  when 4; xco+=0; dir=6   # Left
  when 6; xco-=0; dir=4   # Right
  when 8; yco+=0; dir=2   # Up
  end
...
def pbRegisterPartner(trainerid,trainername,partyid=0)
  ...
  $PokemonGlobal.partner=[trainerid,trainerobject.name,trainerobject.id,trainer[2],trainer[1]]
...
Events.onMapUpdate+=proc {|sender,e|
 ...
 elsif $game_switches[:Devon_Panel_Puzzle]
   if Input.trigger?(Input::X)
...
def pbAutoplayOnTransition
  if $Settings.bike_and_surf_music==1
    if $PokemonGlobal.surfing
      surfbgm=$cache.metadata[:Surf]
      pbBGMPlay(surfbgm) if surfbgm
      return
    end
    if $PokemonGlobal.lavasurfing
      lavasurfbgm=$cache.metadata[:LavaSurf]
      pbBGMPlay(lavasurfbgm) if lavasurfbgm
      return
    end
  end
  ...
  if $PokemonBag.pbStoreItem(item,quantity)   # If item can be picked up
    if pbIsTM?(item)
      Kernel.pbMessage(_INTL("\\se[itemlevel]{1} found \\c[1]{2}\\c[0]!\\nIt contained \\c[1]{3}\\c[0].\\wtnp[30]",
         $Trainer.name,itemname,getMoveName($cache.items[item].checkFlag?(:tm))))
    ...
    if pbIsTM?(item)
      ...
      if quantity>1
        if plural
          Kernel.pbMessage(_INTL("{1} found {2} \\c[1]{3}\\c[0]!\\wtnp[20]", $Trainer.name,quantity,plural))
        else
          Kernel.pbMessage(_INTL("{1} found {2} \\c[1]{3}s\\c[0]!\\wtnp[20]", $Trainer.name,quantity,itemname))
        end
      else
...
def Kernel.pbReceiveItem(item,quantity=1,plural=nil)
  ...
  if pbIsTM?(item)
    Kernel.pbMessage(_INTL("\\se[itemlevel]Obtained \\c[1]{1}\\c[0]!\\nIt contained \\c[1]{2}\\c[0].\\wtnp[30]",
       itemname,getMoveName($cache.items[item].checkFlag?(:tm))))
  elsif (item == :LEFTOVERS)
    Kernel.pbMessage(_INTL("\\se[itemlevel]Obtained some \\c[1]{1}\\c[0]!\\wtnp[30]", itemname))
  elsif quantity>1
    if plural
      Kernel.pbMessage(_INTL("\\se[itemlevel]Obtained {2} \\c[1]{1}\\c[0]!\\wtnp[30]", plural,quantity))
    else
      Kernel.pbMessage(_INTL("\\se[itemlevel]Obtained {2} \\c[1]{1}s\\c[0]!\\wtnp[30]", itemname,quantity))
    end
  else
    Kernel.pbMessage(_INTL("\\se[itemlevel]Obtained \\c[1]{1}\\c[0]!\\wtnp[30]", itemname))
  end
...
#    $game_player.unlock
  ...
  def pbTrainerIntro(symbol)
    pbGlobalLock
    Kernel.pbPlayTrainerIntroME(symbol)
    return true
  end
