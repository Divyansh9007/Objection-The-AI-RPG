...
class PurifyChamberPC
  ...
  def access
    ...
    pbRelicStone()
    #guys i think the feature that hasn't been maintained in 10 years might be broken, crazy right?
    #gonna deal with this some other time, in the meantime have relic stone access in the PC
    #pbPurifyChamber()
  end
end
  ...
  def refresh
    ...
    if pkmn
      if pkmn.type1==pkmn.type2
        textpos.push([_INTL("{1}  Lv{2}  {3}",pkmn.name,pkmn.level,getTypeName(pkmn.type1)),2,0,0,
           Color.new(248,248,248),Color.new(128,128,128)])
      else
        textpos.push([_INTL("{1}  Lv{2}  {3}/{4}",pkmn.name,pkmn.level,getTypeName(pkmn.type1),
           getTypeName(pkmn.type2)),2,0,0,
           Color.new(248,248,248),Color.new(128,128,128)])
      end
