  ...
  def pbGetDexList()
    ...
    for species in $cache.pkmn.keys
      ...
      if pbCanAddForModeList?($PokemonGlobal.pokedexMode,species)
        ...
        dexlist.push([species,$cache.pkmn[species].name,height,weight,$cache.pkmn[species].dexnum,shift])
      end
    end
    return dexlist
  end
    ...
    case $PokemonGlobal.pokedexMode
      when 0 # Numerical mode
        ...
        i=0; loop do break unless i<dexlist.length
          break if $Trainer.pokedex.dexList[dexlist[i][0]][:seen?] || $cache.pkmn[dexlist[i][0]].dexnum < dexlist.length
          dexlist[i]=nil
          i+=1
        end
  ...
  def pbSearchDexList(params)
    ...
    if params[2]!=0 || params[3]!=0 # Filter by type
      ...
      dexlist=dexlist.find_all {|item|
         ...
         type1=$cache.pkmn[item[0]].Type1
         type2=$cache.pkmn[item[0]].Type2
