def pbPurify(pokemon,scene)
  if pokemon.heartgauge==0 && pokemon.shadow
   ...
   # pokemon.giveRibbon(PBRibbons::NATIONAL)
    ...
    for i in 0...4
      if !pokemon.moves[i].nil? && pokemon.moves[i].move != oldmoves[i]
        scene.pbDisplay(_INTL("{1} regained the move \n{2}!",
           pokemon.name,getMoveName(pokemon.moves[i].move)))
      end
    end
    ...
    newexp=PBExp.addExperience(pokemon.exp,pokemon.savedexp||0,pokemon.growthrate)
...
class PokeBattle_Pokemon
  ...
  def makeShadow
    ...
    if $cache.pkmn[self.species].shadowmoves && !$cache.pkmn[self.species].shadowmoves.nil?
      for i in 0...4
        self.shadowmoves[i]=0
        self.shadowmoves[i]=$cache.pkmn[self.species].shadowmoves[i] if $cache.pkmn[self.species].shadowmoves[i]
      end
      self.shadowmovenum=$cache.pkmn[self.species].shadowmoves.length
    else
      # No special shadow moves
      self.shadowmoves[0]=:SHADOWRUSH||0
...
class PokeBattle_Move_131 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    @battle.pbDisplay(_INTL("The darkened sky blocked out the starlight!")) if @battle.FE == :STARLIGHT
    return 0
  end
end
...
class PokeBattle_Move_132 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if @battle.sides[0].effects[:Reflect]>0 ||
       @battle.sides[1].effects[:Reflect]>0 ||
       @battle.sides[0].effects[:LightScreen]>0 ||
       @battle.sides[1].effects[:LightScreen]>0 ||
       @battle.sides[0].effects[:AuroraVeil]>0 ||
       @battle.sides[1].effects[:AuroraVeil]>0 ||
       @battle.sides[0].effects[:AreniteWall]>0 ||
       @battle.sides[1].effects[:AreniteWall]>0 ||
       @battle.sides[0].effects[:Safeguard]>0 ||
       @battle.sides[1].effects[:Safeguard]>0
      pbShowAnimation(@id,attacker,nil,hitnum,alltargets,showanimation)
      @battle.sides[0].effects[:Reflect]=0
      @battle.sides[1].effects[:Reflect]=0
      @battle.sides[0].effects[:LightScreen]=0
      @battle.sides[1].effects[:LightScreen]=0
      @battle.sides[0].effects[:AuroraVeil]=0
