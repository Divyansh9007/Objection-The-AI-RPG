class PokeBattle_Battler
  ...
  TurnEff = [:Bide, :Charge, :Confusion, :Disable, :Embargo, :Encore, 
    ...
    :Telekinesis, :ThroatChop, :Toxic, :Uproar, :Yawn, :TwoTurnAttack]
...
################################################################################
  ...
  def hasCrest?(illusion=false)
    ...
    return false if [:TYPHLOSION,:SAMUROTT,:ELECTRODE].include?(crestmon.species) && crestmon.form!=0
...
################################################################################
  ...
  def pbCheckFormRoundEnd
    ...
    if @battle.field.effect == :DIMENSIONAL && self.ability == :DOWNLOAD
      ...
      self.type1=type
      self.type2=nil
      pbDisplay(_INTL("{1} transformed into the {2} type!",self.pbThis,type.capitalize))
    end
    # Zygarde
    ...
    if (((self.ability == :RKSSYSTEM && self.species == :SILVALLY) || self.crested == :SILVALLY) ||
      ...
      if @battle.field.effect == :NEWWORLD
        ...
        if oldform==0 #Trying to avoid overwriting PBS set forms for them
          ...
          self.form += 19 if $game_switches[:Pulse_Arceus] && self.species == :ARCEUS
        end
      end
  ...
  def pbResetForm
    if !@effects[:Transform]
      if (self.species == :CASTFORM)   ||
        (self.species == :CHERRIM)    ||
        #(self.species == :MELOETTA)   ||
        (self.species == :AEGISLASH && self.form < 2)  ||
        (self.species == :WISHIWASHI) ||
        self.species == :DITTO        ||
        self.species == :MEW          ||
        self.species == :MORPEKO      ||
        self.species == :CRAMORANT
        self.form=0
      elsif (self.species == :DARMANITAN) ||
        (self.species == :MINIOR)
        self.form=@startform
      end
    end
...
################################################################################
  def pbAbilitiesOnSwitchIn(onactive)
    ...
    if Rejuv
      @battle.pbShieldEffects(self,self.onEntryEffects,true) if self.onEntryEffects && onactive
    end
      ...
      if !target.damagestate.substitute
        ...
        if (target.ability == :ILLUSION || target.crested == :ZOROARK) 
          if target.effects[:Illusion]!=nil
            ...
            @battle.pbAnimation(:TRANSFORM,target,target)
            @battle.scene.pbChangePokemon(target,target.pokemon)
...
################################################################################
  ...
  def pbUseBerry(berry=nil,special=false)
    ...
    case berry
      ...
      when :PERSIMBERRY
        if self.status== :FROZEN
          healing = 0 if @effects[:Confusion]>0
          status_berry = true
        end
        status = "confusion"
      when :LUMBERRY
...
################################################################################
  ...
  def pbProcessMoveAgainstTarget(basemove,user,target,numhits,flags={totaldamage: 0},nocheck=false,alltargets=nil,showanimation=true)
    ...
    for i in 0...numhits
      ...
      if target.crested == :BASTIODON
        if target.damagestate.calcdamage>0 && !target.damagestate.substitute &&
          user.ability != (:MAGICGUARD) && !user.isbossmon &&
      ...
      if target.isFainted?
        if target.isbossmon
          if !(target.shieldsBrokenThisTurn > 0)
            target.pbFaint
    ...
    if target.isFainted?
      ...
      if @battle.FE == :BACKALLEY && basemove.function ==0x88 # Pursuit
        if !user.pbTooHigh?(PBStats::SPEED)
          @battle.pbCommonAnimation("StatUp",self,nil)
          user.pbIncreaseStatBasic(PBStats::SPEED,1)
