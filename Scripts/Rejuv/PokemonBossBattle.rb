...
class PokeBattle_Battle
  ...
  def pbShieldDamage(battler,damage=0)
    ...
    if admindam
      ...
      if battler.reconstructcounter >=3
  ...
  def pbShieldEffects(battler,onBreakdata,onEntry=false,delay=false,admin=false)
    ...
    animplay = false
    negativeEffects = [:Curse,:GastroAcid,:Imprison,:Nightmare,:TarShot,:SmackDown,:Encore,:HealBlock,:MultiTurn,:LeechSeed,:Petrification,:Attract]
  ...
  def pbReduceHP(amt,anim=false,emercheck=true)
    ...
    if admindam == true
      ...
      if self.reconstructcounter >=100
        @battle.pbDisplayBrief(_INTL("",self.pbThis))
