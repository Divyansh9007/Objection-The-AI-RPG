...
=end
  ...
  def pbTogglePassword(password, isGameStart=false)
    ...
    if !isGameStart && ['fullivs','easyhms','nohms','hmitems','notmxneeded','freemegaz','shinycharm','earlyshiny','freeexpall','freeremotepc','hello eizen.'].include?(password_string) && checkPasswordActivation(password_string)
      Kernel.pbMessage(_INTL('This password cannot be disabled anymore.'))
      return false
    end
    if !isGameStart && ['randomizer', 'eeveeplease', 'eevee', 'bestgamemode', 'random', 'randomized', 'randomiser', 'randomised','skipintro','nointro','9494','terajuma'].include?(password_string)
    ...
    if ['mintyfresh', 'mintpack'].include?(password_string)
      ...
      Kernel.pbMessage('\PN recieved a set of shiny tools to traverse difficult terrain.')
    elsif ['freemegaz'].include?(password_string)
      $PokemonBag.pbStoreItem(:MEGARING,1)
      Kernel.pbMessage('\PN recieved a Mega-Z Ring!')
    elsif ['freeremotepc'].include?(password_string)
      $PokemonBag.pbStoreItem(:REMOTEPC,1)
      Kernel.pbMessage('\PN recieved a remote PC access.')
      ...
      Kernel.pbMessage('\PN recieved a package of EV-training gear.')
    end
    #pbMonoRandEvents
    return true
  end