...
class FrameCountSlider < Slider
  ...
  def valueupdate(canvas,newvalue)
    @canvas=canvas
    super(_INTL("Frame:"),1,canvas.animation.length,newvalue)
  end
end
...
class AnimationCanvas < Sprite
  ...
  def play(oppmove=false)
    if !@playing
      ...
      pbSpriteSetAnimFrame(@sprites["pokemon0"],
         pbCreateCel(PBScene::FOCUSUSER_X,
                     PBScene::FOCUSUSER_Y,-1,2),
         @sprites["pokemon0"],@sprites["pokemon1"])
      pbSpriteSetAnimFrame(@sprites["pokemon1"],
         pbCreateCel(PBScene::FOCUSTARGET_X,
                     PBScene::FOCUSTARGET_Y,-2,1),
         @sprites["pokemon0"],@sprites["pokemon1"])
      ...
      @player=PBAnimationPlayerX.new(@animation,
         @battle.battlers[oppmove ? 1 : 0],@battle.battlers[oppmove ? 0 : 1],self,nil,oppmove,true)
  ...
  def swapCels(cel1,cel2,frame=@currentframe)
    return if cel1<0 || cel2<0
    return if frame<0 || frame>=@animation.length
    t=@animation[frame][cel1]
    @animation[frame][cel1]=@animation[frame][cel2]
    @animation[frame][cel2]=t
    @currentcel=cel2
    @dirty[cel1]=true
    @dirty[cel2]=true
  end

  def listSelect(canvas,mousex,mousey)
    listSelectable = []
    celselected = false
    for i in 0...PBAnimation::MAXSPRITES
      if pbSpriteHitTest(@celsprites[i],mousex,mousey,false,true)
        listSelectable.push(i)
      end
    end
    if listSelectable.length==0
      Kernel.pbMessage(_INTL("No cels at the clicked position to select."))
      return
    end
    winy=(listSelectable.length*32)+64 #for no. of cels, plus buffer area and cancel button
    listwin=ControlWindow.new(0,0,320,winy)
    listwin.z=99999
    for j in 0...listSelectable.length
      listwin.addButton(_INTL("Cel {1}",listSelectable[j].to_s))
    end
    cancelbutton=listwin.addButton(_INTL("Cancel"))
    loop do
      Graphics.update
      Input.update
      listwin.update
      if listwin.changed?(cancelbutton) || Input.trigger?(Input::B)
        break
      end
      for k in 0...listSelectable.length
        if listwin.changed?(k)
          @currentcel=listSelectable[k]
          invalidate
          celselected=true
        end
        break if celselected==true
      end
      break if celselected==true
    end
    listwin.dispose
  ...
  def addSprite(x,y)
    return false if @currentframe>=@animation.length || @createLock==true
    for i in 0...PBAnimation::MAXSPRITES
      next if @animation[@currentframe][i]
  ...
  def updateInput
    ...
    if currentFrame && !@selecting && (Input.triggerex?(:TAB) || Input.repeatex?(:TAB))
      ...
      currentFrame.length.times {
        if goback==false
          ...
          @currentcel-=1
          @currentcel=currentFrame.length-1 if @currentcel<0
        end
    ...
    if cel
      ...
      if Input.triggerex?(0x51) || Input.repeatex?(0x51)   # Zoom in
      ...
      if Input.triggerex?(0x57) || Input.repeatex?(0x57)   # Zoom out
        cel[AnimFrame::ZOOMX]-=10
      ...
      if Input.triggerex?(0x4E) || Input.repeatex?(0x4E)   # Renumber
        if currentcel && currentcel>=2
          cel1=currentcel
      ...
      if !self.locked?(@currentcel)
        if Input.repeat?(Input::UP)
          increment=(Input.press?(Input::A) ) ? 2 : 8
          snap=(Input.press?(Input::X)) ? true : false
          cel[AnimFrame::Y]-=(cel[AnimFrame::Y]%increment) if snap
        ...
        if Input.repeat?(Input::LEFT)
          increment=(Input.press?(Input::A) ) ? 2 : 8
          snap=(Input.press?(Input::X)) ? true : false
          cel[AnimFrame::X]-=(cel[AnimFrame::X]%increment) if snap
...
def pbConvertSelfTarget(canvas)
  confirmwin=ControlWindow.new(0,0,325,32*7)
  confirmwin.viewport=canvas.viewport
  confirmwin.opacity=200
  confirmwin.addLabel(_INTL("This function is for converting"))
  confirmwin.addLabel(_INTL("self-moves that have the user's"))
  confirmwin.addLabel(_INTL("effects on the target sprite."))
  confirmwin.addLabel(_INTL("Continue?"))
  #Add a checkbox here to move all other cels -256,+128 if necessary for anything, don't think it is but the old code was such a clusterfuck I have no clue anymore
  okbutton=confirmwin.addButton(_INTL("Yes"))
  cancelbutton=confirmwin.addButton(_INTL("No"))
  loop do
    Graphics.update
    Input.update
    confirmwin.update
    if confirmwin.changed?(okbutton) || Input.trigger?(Input::C)
      for frame in 0..canvas.animation.length-1
        usercel=canvas.animation[frame][0]
        targetcel=canvas.animation[frame][1]
        #Setting user effects to target effects
        usercel[AnimFrame::X]=(targetcel[AnimFrame::X]-256)
        usercel[AnimFrame::Y]=(targetcel[AnimFrame::Y]+128)
        usercel[AnimFrame::ZOOMX]=targetcel[AnimFrame::ZOOMX]
        usercel[AnimFrame::ZOOMY]=targetcel[AnimFrame::ZOOMY]
        usercel[AnimFrame::ANGLE]=targetcel[AnimFrame::ANGLE]
        usercel[AnimFrame::OPACITY]=targetcel[AnimFrame::OPACITY]
        usercel[AnimFrame::BLENDTYPE]=targetcel[AnimFrame::BLENDTYPE]
        usercel[AnimFrame::MIRROR]=targetcel[AnimFrame::MIRROR]
        usercel[AnimFrame::PRIORITY]=targetcel[AnimFrame::PRIORITY]
        usercel[AnimFrame::FOCUS]=targetcel[AnimFrame::FOCUS]
        usercel[AnimFrame::COLORRED]=targetcel[AnimFrame::COLORRED]
        usercel[AnimFrame::COLORGREEN]=targetcel[AnimFrame::COLORGREEN]
        usercel[AnimFrame::COLORBLUE]=targetcel[AnimFrame::COLORBLUE]
        usercel[AnimFrame::COLORALPHA]=targetcel[AnimFrame::COLORALPHA]
        usercel[AnimFrame::TONERED]=targetcel[AnimFrame::TONERED]
        usercel[AnimFrame::TONEGREEN]=targetcel[AnimFrame::TONEGREEN]
        usercel[AnimFrame::TONEBLUE]=targetcel[AnimFrame::TONEBLUE]
        usercel[AnimFrame::TONEGRAY]=targetcel[AnimFrame::TONEGRAY]
        #Resetting target to original settings
        targetcel[AnimFrame::X]=384
        targetcel[AnimFrame::Y]=96
        pbResetCel(targetcel)
        #This is where moving other cels will go if needed
      end
      canvas.invalidate
      break
    end
    if confirmwin.changed?(cancelbutton) || Input.trigger?(Input::B)
      break
    end
  end
  confirmwin.dispose
...
def pbRenumberBatch(canvas)
  sliderwin1=ControlWindow.new(0,0,300,32*7)
  sliderwin1.viewport=canvas.viewport
  sliderwin1.opacity=200
  s1set0=sliderwin1.addSlider(_INTL("First Frame:"),1,canvas.animation.length,1)
  s1set1=sliderwin1.addSlider(_INTL("Last Frame:"),1,canvas.animation.length,canvas.animation.length)
  s1set2=sliderwin1.addSlider(_INTL("Old Cel Number:"),2,99,2)
  s1set3=sliderwin1.addSlider(_INTL("New Cel Number:"),2,99,2)
  okbutton=sliderwin1.addButton(_INTL("OK"))
  cancelbutton=sliderwin1.addButton(_INTL("Cancel"))
  loop do
    Graphics.update
    ...
    if sliderwin1.changed?(okbutton) || Input.trigger?(Input::C)
      startframe=sliderwin1.value(s1set0)-1
      endframe=sliderwin1.value(s1set1)-1
      cel1=sliderwin1.value(s1set2)
      cel2=sliderwin1.value(s1set3)
      for i in startframe..endframe
        canvas.swapCels(cel1,cel2,i)
      end
      canvas.invalidate
      break
    end
...
def pbCellBatch(canvas)
  sliderwin1=ControlWindow.new(0,0,280,32*5)
  sliderwin1.viewport=canvas.viewport
  sliderwin1.opacity=200
  s1set0=sliderwin1.addSlider(_INTL("First Frame:"),1,canvas.animation.length,1)
  s1set1=sliderwin1.addSlider(_INTL("Last Frame:"),1,canvas.animation.length,canvas.animation.length)
  s1set2=sliderwin1.addSlider(_INTL("First Cel:"),0,PBAnimation::MAXSPRITES-1,0)
  s1set3=sliderwin1.addSlider(_INTL("Last Cel:"),0,PBAnimation::MAXSPRITES-1,PBAnimation::MAXSPRITES-1)
  sliderwin2=ControlWindow.new(280,0,360,32*17)
  sliderwin2.viewport=canvas.viewport
  sliderwin2.opacity=200
...
def pbHelpWindow
  helptext=""+
     ...
     "E/R - Rotate clockwise/anticlockwise\nP - Open properties screen;\nArrow keys - Move cel 8 pixels "+
     "(hold A - 2 pixels, hold Z - snap)\nQ/W : Zoom in/out\nL - Lock a cel (Prevents it "+
     "from being moved/deleted)\nDEL - Deletes the cel\nN - Renumber cel\nV/B - Move to previous/next "+
     "frame\nSpace - Play animation (Hold A for opp)\nAlso press TAB to switch to the next cel, or hold A "+
     "and press TAB to switch to the previous cel."
...
def pbImportAnim(animations,canvas,animwin)
  ...
  if animfiles.length > 1
    if Kernel.pbConfirmMessage("Multiple .anm files detected. Import all of them?")
      for file in animfiles
        $stdout.print("Importing animation #{file}...")
        begin
          textdata=loadBase64Anim(IO.read(file))
          ...
          $stdout.print("failed\n")
          puts "Animation #{file} is invalid or could not be loaded"
          next
        end
...
def animationEditorMain(animation)
  viewport=Viewport.new(0,0,825,672)
  ...
  sidewin.addButton(_INTL("Lock Creation"))
  sidewin.addSpace
  sidewin.addButton(_INTL("Cel Functions"))
  sidewin.addSpace
  sidewin.addButton(_INTL("Frame Functions"))
  sidewin.addSpace
  ...
  sidewin.addButton(_INTL("Settings"))
  ...
  loop do
    ...
    if Input.trigger?(Input::Y)
      ...
      next
    elsif Input.triggerex?(0x56) || Input.repeatex?(0x56) # V
      canvas.currentframe-=1 if canvas.currentframe>0
      sliderwin.controls[0].curvalue=canvas.currentframe+1
      #This is the code for define path but it's broken so I'm yeeting it until I fix it
      #if canvas.currentCel
      #  pbDefinePath(canvas)
      #  sliderwin.invalidate
      #end
      next
    elsif Input.triggerex?(0x42) || Input.repeatex?(0x42) # B
      canvas.currentframe+=1 if canvas.currentframe<canvas.animation.length-1
      sliderwin.controls[0].curvalue=canvas.currentframe+1
      next
    elsif Input.triggerex?(0x20) #Space
      ...
      case hit
        ...
        when 6 # Select From List
          canvas.listSelect(canvas,mousepos[0],mousepos[1])
        #when 7 # Define Path
          #if canvas.currentCel
            #pbDefinePath(canvas)
            #sliderwin.invalidate
    ...
    if sidewin.changed?(3)
      if canvas.createLock==false
        canvas.createLock=true
        sidewin.controls[3].text="Unlock Creation"
        sidewin.refresh
      else
        canvas.createLock=false
        sidewin.controls[3].text="Lock Creation"
        sidewin.refresh
      end
    end
    ...
    if sidewin.changed?(14)
      pbGlobalAnimSettings(canvas,animation)
    end
  end
  canvas.dispose
  animwin.dispose
...
def pbCelMenu(canvas)
  celwin=ControlWindow.new(420,0,220,512) # Can change if things overlap
  celwin.viewport=canvas.viewport
  celwin.addButton(_INTL("Copy Cels"))
  celwin.addSpace
  celwin.addButton(_INTL("Tweening"))
  celwin.addSpace
  celwin.addButton(_INTL("Cel Batch"))
  celwin.addSpace
  celwin.addButton(_INTL("Flip Cel Rotation"))
  celwin.addSpace
  celwin.addButton(_INTL("Flip Around Point"))
  celwin.addSpace
  celwin.addButton(_INTL("Renumber Cel Batch"))
  loop do
    Graphics.update
    Input.update
    celwin.update
    if (Input.triggerex?(Input::LeftMouseKey) && Mouse::getMousePos[0]<440) || Input.trigger?(Input::B)
      celwin.dispose
      break
    end
    if celwin.changed?(0)
    ...
    if celwin.changed?(6)
      celwin.dispose
      pbCelRotateFlip(canvas)
      break
    end
    if celwin.changed?(8)
      celwin.dispose
      pbFlipAroundPoint(canvas)
      break
    end
    if celwin.changed?(10)
      celwin.dispose
      pbRenumberBatch(canvas)
      break
    end
  end
end

def pbFrameMenu(canvas)
  framewin=ControlWindow.new(440,0,200,512) # Can change if things overlap
  framewin.viewport=canvas.viewport
  framewin.addButton(_INTL("Copy Frames"))
  framewin.addSpace
  framewin.addButton(_INTL("Clear Frames"))
  framewin.addSpace
  framewin.addButton(_INTL("Entire Slide"))
  framewin.addSpace
  framewin.addButton(_INTL("Reverse Animation"))
  framewin.addSpace
  framewin.addButton(_INTL("Convert Old Self-Target"))
  loop do
    Graphics.update
    Input.update
    framewin.update
    ...
    if framewin.changed?(0)
      framewin.dispose
      pbCopyFrames(canvas)
      break
    end
    if framewin.changed?(2)
      framewin.dispose
      pbClearFrames(canvas)
      break
    end
    if framewin.changed?(4)
      framewin.dispose
      pbEntireSlide(canvas)
      break
    end
    if framewin.changed?(6)
      framewin.dispose
      pbReverseAnim(canvas)
      break
    end
    if framewin.changed?(8)
      framewin.dispose
      pbConvertSelfTarget(canvas)
      break
    end
  end
end

def pbGlobalAnimSettings (canvas,animation)
  settingswin=ControlWindow.new(0,0,640,320)
  settingswin.viewport=canvas.viewport
  settingswin.opacity=200
  option1=settingswin.addCheckbox(_INTL("Flip user/target on opp?"))
  option2=settingswin.addCheckbox(_INTL("Flip foreground movement on opp?"))
  okbutton=settingswin.addButton(_INTL("OK"))
  cancelbutton=settingswin.addButton(_INTL("Cancel"))
  settingswin.controls[0].curvalue = animation[animation.selected].oppflip ? animation[animation.selected].oppflip : false
  settingswin.controls[1].curvalue = animation[animation.selected].foreflip ? animation[animation.selected].foreflip : false
  loop do
    Graphics.update
    Input.update
    settingswin.update
    if settingswin.changed?(okbutton)
      animation[animation.selected].oppflip=settingswin.value(option1)
      animation[animation.selected].foreflip=settingswin.value(option2)
      settingswin.invalidate
      break
    end
    if settingswin.changed?(cancelbutton) || Input.trigger?(Input::B)
      break
    end
  end
  settingswin.dispose
    ...
    for i in 0...textdata.length
      ...
      for j in 0...PBAnimation::MAXSPRITES
        ...
        if j==0 # User battler
          ...
          textdata[i][j][AnimFrame::X]=PBScene::FOCUSUSER_X
          textdata[i][j][AnimFrame::Y]=PBScene::FOCUSUSER_Y
        elsif j==1 # Target battler
          textdata[i][j][AnimFrame::FOCUS]=1
          textdata[i][j][AnimFrame::X]=PBScene::FOCUSTARGET_X
          textdata[i][j][AnimFrame::Y]=PBScene::FOCUSTARGET_Y
        else
          textdata[i][j][AnimFrame::FOCUS]=(textdata.position || 4)
          if textdata.position==1
            textdata[i][j][AnimFrame::X]+=PBScene::FOCUSTARGET_X
            textdata[i][j][AnimFrame::Y]+=PBScene::FOCUSTARGET_Y-2
          end
        end
      end
    end
