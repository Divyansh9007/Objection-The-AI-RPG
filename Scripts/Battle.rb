...
################################################################################
 ...
 def pbJudgeSwitch(favorDraws=false)
  ...
  def pbSwitchInName(index,newpoke) #Illusion
    ...
    enemyname = Rejuv ? partynumber[newpoke].name : getMonName(partynumber[newpoke].species)
    if pbIsOpposing?(index)
      newname = illusionpoke != nil ? illusionpoke.name : enemyname
    else
    ...
    if !@opponent
      ...
      if @opponent.is_a?(Array)
        ...
        if sendout1<0 || sendout2<0
          ...
          @battlers[1].pbInitialize(@party2[sendout1],sendout1,false) 
          @battlers[3].pbInitialize(@party2[sendout2],sendout2,false)
          pbDisplayBrief(_INTL("{1} sent\r\nout {2} and {3}!", @opponent.fullname,@party2[sendout1].name,@party2[sendout2].name)) 
          pbSendOut(1,@party2[sendout1])
          pbSendOut(3,@party2[sendout2])
        end
      end
...
################################################################################
  def pbAttackPhase
    ...
    # Goth/Reuni crest type switch
    for i in priority
      next if @choices[i.index][0]!=1 || @choices[i.index][1]<0
      next if (i.crested != :GOTHITELLE && i.crested != :REUNICLUS)
      protype = i.moves[@choices[i.index][1]].type
      next if  protype != :PSYCHIC && !(protype == :DARK && i.species == :GOTHITELLE) && !(protype == :FIGHTING && i.species == :REUNICLUS)
      prot1 = i.type1
      prot2 = i.type2
...
################################################################################
  def pbEndOfRoundPhase
        ...
        if @weatherduration==0
          ...
          if pbWeather== :SANDSTORM
            ...
            for i in priority
              ...
              i.effects[:DesertsMark] # Desert's mark sand immunity negation
    ...
    for i in priority
      ...
      next if i.isFainted?
      if i.chargeAttack
        return if (i.status == :SLEEP || i.status == :FREEZE)
