...
def pbPokemonBitmap(species, shiny=false, back=false, gender=nil, form = 0, looped = false)   # Used by the Pokédex
  ...
  bitmapFileName=sprintf("Graphics/Battlers/%03d%s",$cache.pkmn[species].dexnum,gendermod)
  bitmapFileName=sprintf("Graphics/Battlers/%03d",$cache.pkmn[species].dexnum) if !pbResolveBitmap(bitmapFileName)
...
def pbLoadPokemonBitmapSpecies(pokemon, species, back=false)
  ...
  if (pokemon.form !=0 && (ShitList.include?(species))) || toobig
    if !(species == :STEELIX && pokemon.form == 1)  #mega steelix is excused.
      ...
      if !pbResolveBitmap(bitmapfilename2)
        bitmapfilename2 = sprintf("Graphics/Battlers/%03d%s%s",dexnum,shinytag,backtag)
      end
      if pbResolveBitmap(bitmapfilename2)
  ...
  if pokemon.isEgg?
    ...
    bitmapFileName=pbCheckPokemonBitmapFiles(dexnum,pokemon.isFemale?)
    bitmapFileName=pbCheckPokemonBitmapFiles(dexnum,false) if !pbResolveBitmap(bitmapFileName)
  ...
  if spritesheet.height <= y && pokemon.isFemale? && !pokemon.isEgg?
    bitmapFileName=pbCheckPokemonBitmapFiles(dexnum)
    spritesheet = RPG::Cache.load_bitmap(bitmapFileName)
    bitmap=Bitmap.new(height,height)
  end
...
def pbCheckPokemonBitmapFiles(dexnum,girl=false)
  gendermod = girl ? "_f" : ""
  ...
  ret=pbResolveBitmap(bitmapFileName)
  return ret if ret
  name = $cache.pkmn.values[dexnum-1].name
  bitmapFileName=sprintf("Graphics/Battlers/%s%s",name,gendermod)
  ret=pbResolveBitmap(bitmapFileName)
  return ret if ret
  bitmapFileName=sprintf("Graphics/Battlers/%s",name)
  ret=pbResolveBitmap(bitmapFileName)
  return ret if ret
...
def pbPokemonIconBitmap(pokemon,egg=false)   # pbpokemonbitmap, but for icons
  ...
  name = $cache.pkmn[pokemon.species].name.downcase
  filename=sprintf("Graphics/Icons/%03d",species) if form == 0
  filename=sprintf("Graphics/Icons/%03d_%s",species,form) if !pbResolveBitmap(filename)
  filename=sprintf("Graphics/Icons/%03d%s",species,girl) if !pbResolveBitmap(filename)
  filename=sprintf("Graphics/Icons/icon%03d%s%s", species,girl,egg) if !pbResolveBitmap(filename)
  filename=sprintf("Graphics/Icons/icon%03d_%s%s", species,form,egg) if !pbResolveBitmap(filename)
  filename=sprintf("Graphics/Icons/icon%03d%s", species,egg) if !pbResolveBitmap(filename)
  filename=sprintf("Graphics/Icons/%s%s",name,girl) if !pbResolveBitmap(filename)
  filename=sprintf("Graphics/Icons/%s",name) if !pbResolveBitmap(filename)
