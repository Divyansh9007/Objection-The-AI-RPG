...
################################################################################
  ...
  def pbDragonDartTargetting(attacker) # all of this just to get dragon darts targeting right, i hate it here
    ...
    if (darttype = :WATER || fieldsecondtype == :WATER) && (([:DRYSKIN,:WATERABSORB,:STORMDRAIN].include?(opp2.ability) && !opp2.moldbroken) || (Rejuv && @battle.FE == :GLITCH && opp2.species == :GENESECT && opp2.hasWorkingItem(:DOUSEDRIVE)))
      return [opp1]
    end
    if (darttype = :WATER || fieldsecondtype == :WATER) && (([:DRYSKIN,:WATERABSORB,:STORMDRAIN].include?(opp1.ability) && !opp1.moldbroken) || (Rejuv && @battle.FE == :GLITCH && opp1.species == :GENESECT && opp1.hasWorkingItem(:DOUSEDRIVE)))
      return [opp2]
    end
    if (darttype = :ELECTRIC || fieldsecondtype == :ELECTRIC) && (([:MOTORDRIVE,:VOLTABSORB,:LIGHTNINGROD].include?(opp2.ability) && !opp2.moldbroken) || (Rejuv && @battle.FE == :GLITCH && opp2.species == :GENESECT && opp2.hasWorkingItem(:SHOCKDRIVE)))
      return [opp1]
    end
    if (darttype = :ELECTRIC || fieldsecondtype == :ELECTRIC) && (([:MOTORDRIVE,:VOLTABSORB,:LIGHTNINGROD].include?(opp1.ability) && !opp1.moldbroken) || (Rejuv && @battle.FE == :GLITCH && opp1.species == :GENESECT && opp1.hasWorkingItem(:SHOCKDRIVE)))
      return [opp2]
    end
    if (darttype = :FIRE || fieldsecondtype == :FIRE) && ((((opp1.ability == :FLASHFIRE && @battle.FE != :FROZENDIMENSION) || (opp1.ability == :MAGMAARMOR && [:VOLCANICTOP,:DRAGONSDEN,:INFERNAL].include?(@battle.FE))) && !opp1.moldbroken) || (Rejuv && @battle.FE == :GLITCH && opp1.species == :GENESECT && opp1.hasWorkingItem(:BURNDRIVE)) || opp1.crested == :DRUDDIGON)
      return [opp2]
    end
    if (darttype = :FIRE || fieldsecondtype == :FIRE) && ((((opp2.ability == :FLASHFIRE && @battle.FE != :FROZENDIMENSION) || (opp2.ability == :MAGMAARMOR && [:VOLCANICTOP,:DRAGONSDEN,:INFERNAL].include?(@battle.FE))) && !opp2.moldbroken) || (Rejuv && @battle.FE == :GLITCH && opp2.species == :GENESECT && opp2.hasWorkingItem(:BURNDRIVE)) || opp2.crested == :DRUDDIGON)
      return [opp1]
    end
    if (darttype = :GRASS || fieldsecondtype == :GRASS) && ((opp1.ability == :SAPSIPPER && !opp1.moldbroken) || opp1.crested == :WHISCASH)
      return [opp2]
    end
    if (darttype = :GRASS || fieldsecondtype == :GRASS) && ((opp2.ability == :SAPSIPPER && !opp2.moldbroken) || opp2.crested == :WHISCASH)
      return [opp1]
    end
    if (darttype = :ICE || fieldsecondtype == :ICE) && @battle.FE == :GLITCH && opp2.species == :GENESECT && opp2.hasWorkingItem(:CHILLDRIVE) && !opp2.moldbroken
      return [opp1]
    end
    if (darttype = :ICE || fieldsecondtype == :ICE) && @battle.FE == :GLITCH && opp1.species == :GENESECT && opp1.hasWorkingItem(:CHILLDRIVE) && !opp1.moldbroken
      return [opp2]
    end
    if (darttype = :GROUND || fieldsecondtype == :GROUND) && (opp2.isAirborne? || pp2.crested == :SKUNTANK)
      return [opp1]
    end
    if (darttype = :GROUND || fieldsecondtype == :GROUND) && (opp1.isAirborne? || opp1.crested == :SKUNTANK)
      return [opp2]
    end
...
################################################################################
  ...
  def pbCalcDamage(attacker,opponent,options=0, hitnum: 0)
    ...
    finalmult*=0.5 if opponent.effects[:Shelter] && @battle.FE != :INDOOR && (type == @battle.field.mimicry || self.getSecondaryType(attacker) == @battle.field.mimicry)
